"""Hermetic unit tests for the in-process session store (Phase 5 login)."""
...
class TestExpiry:
    def test_expired_token_is_invalid_and_removed(self):
        ...
        store._sessions[token] = datetime.now(timezone.utc) - timedelta(seconds=1)
    ...
    def test_token_still_valid_just_before_expiry(self):
        store = SessionStore(ttl_minutes=10)
        token = store.create()
        store._sessions[token] = datetime.now(timezone.utc) + timedelta(seconds=5)
        assert store.is_valid(token) is True
